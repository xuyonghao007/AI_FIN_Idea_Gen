...
def main_workflow():
    ...
    for run in range(1, TOTAL_RUNS + 1):
        ...
        if run % 5 == 0:
            ...
            # 保存所有想法
            all_ideas_path = f"{OUTPUT_DIR}/all_ideas_current.json"
            write_json(all_ideas_path, sorted_ideas)
            print(f"所有想法已保存到: {all_ideas_path}")
            
            # 保存前10个想法
            top_ideas_path = f"{OUTPUT_DIR}/top_10_ideas_current.json"
            ...
            top_ideas_text_path = f"{OUTPUT_DIR}/top_10_ideas_current.md"
    ...
    # 更新当前结果文件
    all_ideas_current_path = f"{OUTPUT_DIR}/all_ideas_current.json"
    write_json(all_ideas_current_path, sorted_ideas)
    print(f"当前所有想法已更新到: {all_ideas_current_path}")
    
    top_ideas_current_path = f"{OUTPUT_DIR}/top_10_ideas_current.json"
    write_json(top_ideas_current_path, top_ideas)
    print(f"当前前10个最高分的想法已更新到: {top_ideas_current_path}")
    
    top_ideas_text_current_path = f"{OUTPUT_DIR}/top_10_ideas_current.md"
    write_file(top_ideas_text_current_path, top_ideas_text)
    print(f"当前前10个最高分的想法(文本版)已更新到: {top_ideas_text_current_path}")
